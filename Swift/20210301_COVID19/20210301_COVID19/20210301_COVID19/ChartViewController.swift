...
class ChartViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        gradientLayer.colors = [colors.bluePerple.cgColor,
                                colors.blue.cgColor,]
        ...
        backButton.addTarget(self, action: #selector(backButtonAcction), for: .touchUpInside)
        ...
        segment.setTitleTextAttributes([NSAttributedString.Key.foregroundColor: colors.bluePerple], for: .normal)
        ...
        bottomLabel(uiView, prefectures, 1, 10, text: "東京", size: 30, weight: .ultraLight, color: colors.black)
        bottomLabel(uiView, pcr, 0.39, 50, text: "PCR数", size: 15, weight: .bold, color: colors.bluePerple)
        bottomLabel(uiView, pcrCount, 0.39, 85, text: "2222222", size: 30, weight: .bold, color: colors.blue)
        bottomLabel(uiView, cases, 1, 50, text: "感染者数", size: 15, weight: .bold, color: colors.bluePerple)
        bottomLabel(uiView, casesCount, 1, 85, text: "22222", size: 30, weight: .bold, color: colors.blue)
        bottomLabel(uiView, deaths, 1.61, 50, text: "死者数", size: 15, weight: .bold, color: colors.bluePerple)
        ...
        for i in 0..<CovidSingleton.shared.prefecture.count {
            if CovidSingleton.shared.prefecture[i].name_ja == "東京" {
                prefectures.text = CovidSingleton.shared.prefecture[i].name_ja
        ...
        chartView.leftAxis.labelTextColor = colors.bluePerple
    ...
    @objc func switchAction(sendar: UISegmentedControl) {
        switch sendar.selectedSegmentIndex {
        case 0:
            pattern = "cases"
        case 1:
            pattern = "pcr"
    ...
    func bottomLabel(_ parentView: UIView, _ label: UILabel, _ x: CGFloat, _ y: CGFloat, text: String, size: CGFloat, weight: UIFont.Weight, color: UIColor) {
        ...
        label.frame = CGRect(x: x, y: y, width: parentView.frame.size.width / 3.5, height: 50)
...
extension ChartViewController: UISearchBarDelegate {
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        if let index = array.firstIndex(where: { $0.name_ja == searchBar.text}) {
            prefectures.text = "\(array[index].name_ja)"
