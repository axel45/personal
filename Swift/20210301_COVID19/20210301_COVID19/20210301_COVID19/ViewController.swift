...
class ViewController: UIViewController {
    ...
    func setUpGradation() {
        ...
        gradiertLayer.frame = CGRect(x: 0, y: 0, width: view.frame.size.width, height: view.frame.size.height / 2)
        gradiertLayer.colors = [colors.bluePerple.cgColor, colors.blue.cgColor]
        gradiertLayer.startPoint = CGPoint(x: 0, y: 0)
        gradiertLayer.endPoint = CGPoint(x: 1, y: 1)
        view.layer.insertSublayer(gradiertLayer, at: 0)
    }
}
