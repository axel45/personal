...
class HealthCheckViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        calendar.appearance.headerTitleColor = colors.bluePerple
        calendar.appearance.weekdayTextColor = colors.bluePerple
        ...
        creteImage(parentView: uiView1, imageName: "check1")
        createLabel(parentView: uiView1, text: "37.5度以上の熱がある")
        createUISwitch(parentView: uiView1, action: #selector(swithcAction))
        let uiView2 = createView(y: 465)
        scrollView.addSubview(uiView2)
        creteImage(parentView: uiView2, imageName: "check2")
        createLabel(parentView: uiView2, text: "のどの痛みがある")
        createUISwitch(parentView: uiView2, action: #selector(swithcAction))
        let uiView3 = createView(y: 550)
        scrollView.addSubview(uiView3)
        creteImage(parentView: uiView3, imageName: "check3")
        createLabel(parentView: uiView3, text: "匂いを感じない")
        createUISwitch(parentView: uiView3, action: #selector(swithcAction))
        let uiView4 = createView(y: 635)
        scrollView.addSubview(uiView4)
        creteImage(parentView: uiView4, imageName: "check4")
        createLabel(parentView: uiView4, text: "味が薄く感じる")
        createUISwitch(parentView: uiView4, action: #selector(swithcAction))
        let uiView5 = createView(y: 720)
        scrollView.addSubview(uiView5)
        creteImage(parentView: uiView5, imageName: "check5")
        createLabel(parentView: uiView5, text: "だるさがある")
        createUISwitch(parentView: uiView5, action: #selector(swithcAction))
    ...
    func creteImage(parentView: UIView, imageName: String) {
    ...
    func createView(y: CGFloat) -> UIView {
        ...
        uiView.layer.shadowOffset = CGSize(width: 0, height: 12)
        return uiView
    }
}
...
extension HealthCheckViewController: FSCalendarDataSource, FSCalendarDelegate, FSCalendarDelegateAppearance {
    ...
    func calendar(_ calendar: FSCalendar, subtitleFor date: Date) -> String? {
        if let result = UserDefaults.standard.string(forKey: deteFormatter(day: date)) {
            return result
        }
        return ""
    }
    ...
    func calendar(_ calendar: FSCalendar, appearance: FSCalendarAppearance, borderDefaultColorFor date: Date) -> UIColor? {
        // カレンダーの日付 == 今日の日付
        if deteFormatter(day: date) == today {
            return colors.bluePerple
        }
        return .clear
    }
    ...
    func deteFormatter(day: Date) -> String {
